process.env.BOT_TOKEN = "NjQ2NjAwMzUyMzkyNjc1MzI4.XdUHWw.aAgXiSZJM--h90jwMJWt08fKLHM";
process.env.YTAPI = "AIzaSyBF7K9Or1BEDzbfpIMV-Yvbr9sLSROZ-QI";
...
const VIDEOCACHE_FILE = "./videocache.dat";
var fs = require('fs');
    ...
    if (response) {
        ...
        switch(args[0]) {
            ...
            case 'lastvideo':
                if (args.length > 1) {
                    if (args[1] === "debug") {
                        message.channel.send("Last video ID: " + lastVideo);
                        return;
                    }
                    getLastVideo(null, true);
                    return;
                }
                getLastVideo(message.channel, false);
                break;
            case 'purge':
 ...
 */
function getLastVideo(channel, check) {
    ...
    service.playlistItems.list({
        ...
        if (videos.length == 0) {
            ...
            // Execute check if needed.
            if (check === true) {
                // Check if last video equals video ID
                if (lastVideo === videoId) {
                    return;
                } else {
                    // Update and save.
                    lastVideo = videoId;
                    writeLastVideo(videoId);
                }
            }
            ...
            if (channel == null) {
                channel = bot.channels.find(c => c.name === "youtube-channel");
            }
...
function writeLastVideo(data) {
    fs.writeFile(VIDEOCACHE_FILE, data, 'utf8', (err) => {
        if (err) throw err;
